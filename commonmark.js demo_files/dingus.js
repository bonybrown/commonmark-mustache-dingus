...
var parseAndRender = function() {
    var textarea = $("#text");
    var startTime = new Date().getTime();
    var templateInput = textarea.val();
    var parsed = templateInput;
    var inputData = {};
    try{
        $("#datawarnings")[0].innerText = "";
        var datatext = $("#data").val();
        var data = JSON.parse(datatext);
    }catch(e){
        $("#datawarnings")[0].innerText = e;
    }
    try{
        $("#warnings")[0].innerText = "";
        var md = Mustache.render(templateInput,data);
        parsed = reader.parse(md);
